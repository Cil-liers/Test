...
public class MySolution extends RecursiveTask<Boolean> {
	...
	public Boolean compute() {
		System.out.println("The area is " + (rows-2)*(columns-2));
		if ((rows-2)*(columns-2) <= 9) {
			...
			MySolution topRight = new MySolution(split,columns,1,split-1,newGrid,emptyGrid);
			// MySolution bottomLeft = new MySolution(rows,split,split-1,1,newGrid,emptyGrid);
			// MySolution bottomRight = new MySolution(rows,columns,split-1,split-1,newGrid,emptyGrid);
